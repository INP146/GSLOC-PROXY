...
`.env` 控制进程启动参数，例如代理端口、管理 Web/API 监听地址、策略文件路径、运行时状态路径和 mitmproxy 配置目录。

### `policy.json`

`policy.json` 定义代理允许处理的 host 和 path。公开版本应保持严格 allowlist，不应扩展为通用代理。

### `state.json`

`state.json` 保存运行时实验状态，包括是否启用、模拟上游位置、信号变换模式和缩放参数。
...
仓库中的 `sing-box-1.13-gsloc.json` 是本地实验路由示例。请根据你的授权测试环境调整地址、端口和网络范围。
